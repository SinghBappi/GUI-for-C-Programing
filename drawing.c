...
static void resize_cb(GtkWidget *widget, int width, int height, gpointer data) {
    ...
    if (gdk_surface) {
        cairo_surface_t *cairo_surface = gdk_surface_create_similar_surface(gdk_surface, CAIRO_CONTENT_COLOR, width, height);
        surface = cairo_surface_create_similar(cairo_surface, CAIRO_CONTENT_COLOR, width, height);
